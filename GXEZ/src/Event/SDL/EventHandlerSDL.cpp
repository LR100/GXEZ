...
#include <iostream>
	...
	{
		_evtDef.type = Event::QUIT;
	}

	void EventHandlerSDL::HandleTypeKeyDown(const SDL_Event& event)
	{
		if (_keysLink.count(event.key.keysym.sym))
			_evtDef.key = _keysLink.at(event.key.keysym.sym);
		_evtDef.type = Event::KEY;
		_evtDef.state = ControlKeyState::PRESSED;
	}
	...
	void EventHandlerSDL::HandleTypeKeyUp(const SDL_Event& event)
	{
		if (_keysLink.count(event.key.keysym.sym))
			_evtDef.key = _keysLink.at(event.key.keysym.sym);
		_evtDef.type = Event::KEY;
		_evtDef.state = ControlKeyState::RELEASED;
	}

	void EventHandlerSDL::HandleTypeMouseWheel(const SDL_Event& event)
	{
		_evtDef.type = Event::MOUSE;
		_evtDef.key = ControlKey::MOUSE_SCROLL;
		_evtDef.state = Event::GetDefaultControlKeyStateFromControlKey(_evtDef.key);
	...
	void EventHandlerSDL::HandleTypeMouseButtonDown(const SDL_Event& event)
	{
		...
		if (_mouseLink.count(event.button.button))
			_evtDef.key = _mouseLink.at(event.button.button);
		_evtDef.state = ControlKeyState::REMAINED;
		// Correct Specification of SDL Handling by Retaining mouse pressed
		//std::cout << "Mouse Down" << std::endl;
		_pressedRetainer[_evtDef.key] = _evt;
		_evtDef.state = ControlKeyState::PRESSED;
	}
	...
	void EventHandlerSDL::HandleTypeMouseButtonUp(const SDL_Event& event)
	{
		...
		if (_pressedRetainer.count(_evtDef.key))
			_pressedRetainer.erase(_evtDef.key);
	...
	{
		_evtDef.type = Event::MOUSE;
		_evtDef.key = ControlKey::MOUSE_MOTION;
		_evtDef.state = Event::GetDefaultControlKeyStateFromControlKey(_evtDef.key);
	}

	void EventHandlerSDL::HandleTypeWindow(const SDL_Event& event)
	{
		_evtDef.type = Event::WINDOW;
		_evtDef.idDevice = event.window.windowID;
		if (_windowLink.count(event.window.event))
		{
			_evtDef.key = _windowLink.at(event.window.event);
			_evtDef.state = Event::GetDefaultControlKeyStateFromControlKey(_evtDef.key);
		}
		_evt.SetDefinition(_evtDef);
